 ...
 * `127.0.0.1:2181`. You can specify a different ZooKeeper instance by setting the `zookeeper.connect` parameter 
...
class KafkaEmbedded(config:Properties=new Properties) extends LazyLogging {
  ...
  private val logDir = {
    ...
    val path = Seq(System.getProperty("java.io.tmpdir"), "kafka-piwik", "logs-" + random).mkString(File.separator)
    new File(path)
