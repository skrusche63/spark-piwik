...
object NPrefBuilder {
   ...
   * 
   */  
  def build(dataset:RDD[String]):RDD[Preference] = {

    val transactions = dataset.map(data => {
     
      val Array(idsite,user,idorder,timestamp,items) = data.split("\\|")
      val cid = idsite + "|" + user
  ...
  def compute(transactions:RDD[(String,String)]):RDD[Preference] = {
    /**
     * STEP #1
     * 
     ...
     */
    userItemPref.keyBy(valu => valu._2).join(itemMaxPref)
      .map(valu => {
