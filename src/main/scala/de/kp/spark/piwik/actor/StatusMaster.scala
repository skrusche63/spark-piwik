...
class StatusMaster(@transient val sc:SparkContext) extends MonitoredActor with ActorLogging {

  val router = context.actorOf(Props(new StatusActor(sc)).withRouter(RoundRobinRouter(workers)))
